    ...
    public function index(Request $request)
    {
        ...
        if (!empty($no_kp) && !empty($ipt_id)) {            
            ...
            ->paginate(10);              
             ...
             Session::put('no_kp', $no_kp);             
             $no_kp = Session::get('no_kp');

           
                $users = User::where('no_kp', 'LIKE', '%'.$no_kp.'%')
                ->orderBy('name', 'ASC')
                ->paginate(10); 
         } else {
      ...
      // dd($users);
        return view('student.index',compact('users','ipt_id','no_kp'));
    }
...
//PDF::Cell(0, 0, 'A4 PORTRAIT', 1, 1, 'C');
        ...
        PDF::SetX(0); // abscissa of Horizontal position 
	PDF::MultiCell(0, 0, $txt, 0, 'R', 0, 0, '', '', true, 0, false, true, 40, 'T');
    //PDF::MultiCell(0, 0, $txt,1,1,'L',false);

    //$html = '<div style="position: absolute; top:0px !important; right:300px;">'.$txt.'</div>';
...
//        PDF::Write(0, $txt, '', 0, 'R', true, 0, false, false, 0);
        ...
        //PDF::writeHTML($barcode, '', 5, 100, 80, '', '', 'M');
         //PDF::Image($url, '80', 80, '', '70', 'jpg', '', '', false, 100, '', false, false, 0, false, false, false);
       //  'kiri/kanan', 'atas/bawah',' ', ''
       PDF::Image($url,'70', 80, '', '115', 'jpg', '', '', 'false', 90, '', false, false, 0, false, false, false);
       //PDF::Image($url, '', 0, 'R', true, 0, false, false, 0);
